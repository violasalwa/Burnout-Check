...
<style>
/* ============================================================
   admin/soal/index.blade.php — Custom CSS
...
@media (max-width: 768px) {
    ...
    .table tbody td:nth-child(5) { display: table-cell; }
    nav[role="navigation"] {
        flex-wrap: wrap;
        gap: 4px;
        padding: 0 1.25rem;
...
{{-- CARD TAMBAH SOAL --}}
<div class="card">
    <a href="{{ route('admin.soal.create') }}" class="btn btn-success">
        + Tambah Soal
    </a>
</div>
        ...
        <tbody>
            @foreach ($soals as $index => $soal)
                <tr>
                    ...
                    <td>{{ \Illuminate\Support\Str::limit($soal->pertanyaan, 50) }}</td>
                    <td>{{ $soal->kategori }}</td>
                    <td>
                        <span class="badge"
                            style="background: {{ $soal->is_active ? '#d4edda' : '#f8d7da' }};
                                   color: {{ $soal->is_active ? '#155724' : '#721c24' }};">
                    ...
                    <td>
                        ...
                        <form action="{{ route('admin.soal.destroy', $soal->id) }}"
                              method="POST"
